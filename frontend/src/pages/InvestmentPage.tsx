...
// future value with annual additions, annual compounding
...
export default function InvestmentPage() {
    ...
    // expenses from LandingPage (localStorage)
    ...
    useEffect(() => {
        ...
        setAnnualAdd(Number(annualExpendable.toFixed(0)));            // invest expendable
    ...
    const calculate = () => {
        ...
        setSeries(points);
        setFuture(points[points.length - 1].value);
    };

    return (
        <Container maxWidth="md" sx={{ mt: 4 }}>
            <Typography variant="h4" gutterBottom>Compounding Interest Simulator</Typography>
            ...
            <Paper sx={{ p: 3, mb: 4 }} elevation={3}>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}><TextField label="Principal ($)" type="number" fullWidth value={principal} onChange={e => setPrincipal(+e.target.value)} /></Grid>
                    <Grid item xs={12} sm={6}><TextField label="Annual Contribution ($)" type="number" fullWidth value={annualAdd} onChange={e => setAnnualAdd(+e.target.value)} /></Grid>
                    <Grid item xs={12} sm={6}><TextField label="Annual % Return" type="number" fullWidth value={ratePct} onChange={e => setRatePct(+e.target.value)} /></Grid>
                    <Grid item xs={12} sm={6}><TextField label="Years" type="number" fullWidth value={years} onChange={e => setYears(+e.target.value)} /></Grid>

                    <Grid item xs={12}><Button variant="contained" onClick={calculate}>Calculate</Button></Grid>
                </Grid>
            </Paper>
            ...
            {future !== null && (
                <>
                    <Paper sx={{ p: 3, mb: 3 }} elevation={2}>
                        <Typography variant="h6">Future value after {years} years:</Typography>
                        <Typography variant="h4">${future.toLocaleString(undefined, { maximumFractionDigits: 0 })}</Typography>
                    </Paper>

                    {/* chart */}
                    <Paper sx={{ p: 2 }} elevation={2}>
                        <Typography variant="subtitle1" gutterBottom>Growth over time</Typography>
                        <Box sx={{ height: 320 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={series} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                                    ...
                                    <Tooltip formatter={(v: number) => `$${v.toLocaleString(undefined,{maximumFractionDigits:0})}`} labelFormatter={l => `Year ${l}`} />
